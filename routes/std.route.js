...
stdRoute.post("/create-std", async (req, res) => {
  try {
    ...
    // ✅ ลบ role_id ออก
    const internToUser = `INSERT INTO users (username, password) 
                   VALUES ($1, $2) RETURNING *`;
    const query = `INSERT INTO students (fullname, std_class_id, username, password, major) 
                   VALUES ($1, $2, $3, $4, $5) RETURNING *`;

    const insertUser = await pool.query(internToUser, [username, password]);
    ...
    if (!result) return res.status(400).json({ err: "สร้างนักเรียนไม่สำเร็จ" });
...
// ✅ แก้ไขส่วนนี้
// ✅ แก้ไขส่วนนี้ - ไม่ใช้ตาราง classes
stdRoute.post("/check-class", upload.single("leavDoc"), async (req, res) => {
  try {
    ...
    // 🔸 กำหนดเวลาเรียน (แก้ตามเวลาจริง)
    const startTime = "09:00:00";
    const checkInTimeOnly = checkInTime.toTimeString().slice(0, 8);

    // ✅ ตรวจสอบสถานะ: ถ้ามีไฟล์ลา = "ลา", ถ้าไม่มีดูเวลา
    let status;
    if (filePath) {
      status = "ลา";  // มีไฟล์ลา
    } else {
      status = checkInTimeOnly > startTime ? "มาสาย" : "ตรงเวลา";
    }
    ...
    const result = await pool.query(query, [classId, stdId, status, filePath]);
